...
class RuleBasedDetector:
    ...
    def detect_complex_sentences(self) -> List[Dict[str, Any]]:
        ...
        for i, line in enumerate(self.lines):
            ...
            # IMPORTANT: Exclude commas inside mathematical expressions <m>...</m> and <me>...</me>
            # Everything between <m> and </m> is one mathematical object
            text_without_math = re.sub(r'<m>.*?</m>', '', content)
            text_without_math = re.sub(r'<me>.*?</me>', '', text_without_math)
            ...
            if comma_count >= 3:
                if self.should_flag(1):
                    findings.append({
                        ...
                        "line_numbers": [line_num],
                        "quoted_text": content[:100] + "..." if len(content) > 100 else content,
                        "category": "Conceptual Clarity",
                        "severity": 1,
                        "student_impact": "Complex sentences are harder for struggling readers",
                        "suggested_fix": "Break into simpler sentences",
                        "confidence": 0.55
                    })
...
def generate_html_report(consensus_issues: List[Dict[str, Any]],
    ...
    def text_to_numbered_html_with_latex(text):
        ...
        for line in lines:
            ...
            # Restore LaTeX tags
            escaped = escaped.replace('___LATEX_M_START___', '<m>')
            escaped = escaped.replace('___LATEX_M_END___', '</m>')
            escaped = escaped.replace('___LATEX_ME_START___', '<me>')
            escaped = escaped.replace('___LATEX_ME_END___', '</me>')
    ...
    # Display the ORIGINAL XML (module_content is now the raw XML string)
    # Escape it for HTML display and format with line numbers
    import html as html_escape_module
    escaped_xml = html_escape_module.escape(module_content)

    # Add line numbers to XML
    xml_lines = escaped_xml.split('\n')
    numbered_lines = []
    for i, line in enumerate(xml_lines, 1):
        # Add line numbers with proper formatting
        numbered_lines.append(f'<div style="font-family: monospace; margin: 0;"><span style="color: #666; margin-right: 10px; user-select: none;">{i:4d}</span>{line}</div>')
...
<body>
    <div class="container">
        ...
        <div id="original-input" class="tab-content">
            <h2>Original Module XML</h2>
            <p>Line-numbered source XML exactly as inputted. This is the ORIGINAL, UNMODIFIED module content.</p>

            <div class="original-input">{escaped_module}</div>
        </div>
...
def _format_issues_html(issues: List[Dict[str, Any]], total_agents: int,
    ...
    def escape_preserve_latex(text):
        ...
        # Restore LaTeX tags
        text = text.replace('___LATEX_M_START___', '<m>')
        text = text.replace('___LATEX_M_END___', '</m>')
        text = text.replace('___LATEX_ME_START___', '<me>')
        text = text.replace('___LATEX_ME_END___', '</me>')
