...
class ConsensusAggregator:
    ...
    def _are_similar(self, feedback1: ReviewFeedback, feedback2: ReviewFeedback) -> bool:
        ...
        # Combine factors
        # MINIMAL GROUPING to err on side of false positives (human in the loop)
        # ONLY group if issue descriptions are nearly identical
        # Let humans decide what's a duplicate - our job is to flag everything

        # Single criterion: Nearly identical issue descriptions
        # This only catches true duplicates (same agent flagging exact same thing)
        if issue_similarity > self.similarity_threshold:
            return True

        # Do NOT group based on type/location - preserve all distinct observations
        return False
