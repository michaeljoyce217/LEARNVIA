    ...
    def __init__(self, config_dir: Optional[str] = None):
        ...
        Args:
            config_dir: Path to the configuration directory.
                       If None, uses config/ in project root (recommended).
        """
...
class BaseReviewer:
    ...
    def _load_guidelines_file(self, filename: str) -> str:
        ...
        # Try config/prompts first (new structure)
        file_path = project_root / "config" / "prompts" / filename

        # Try config directly as fallback
        if not file_path.exists():
